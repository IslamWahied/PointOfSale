...
{
    public partial class Unit_Back_office : DevExpress.XtraEditors.XtraForm
    {
     ...
     //  readonly POSEntity context = new POSEntity();
        ...
        void langu()
        {
            ...
            gridColumn1.Caption = st.isEnglish() ? "Unit Name" : "اسم الوحدة";
            gridColumn2.Caption = st.isEnglish() ? "Unit Name" : "اسم الوحدة";

            materialLabel1.Text = st.isEnglish() ? "Unit Name" : "اسم الوحدة";
        ...
        List<UnitCard_Back_Office> _list_Unit_Back_Office = new List<UnitCard_Back_Office>();
        ...
        {
            List<UnitCard> _ItemCardView = new List<UnitCard>();

            using (DataRep.POSEntity _context23 = new DataRep.POSEntity())
            {
                var GetAllCategory = _context23.UnitCards.Where(x => x.IsDeleted == 0 && x.Branch_Code == 0).ToList();
        ...
        private void windowsUIButtonPanel_ButtonClick(object sender, DevExpress.XtraBars.Docking2010.ButtonEventArgs e)
        {
             ...
             if (btn.Caption == "حفظ" || btn.Caption == "Save")
            {
                ...
                try
                {
                    ...
                    _list_Unit_Back_Office.Where(x => x.IsWindow_Transfer).ToList().ForEach(x => {
                        ...
                        using (db_a8f74e_posEntities Contexts2 = new db_a8f74e_posEntities())
                        {
                            ...
                            if (x.Event_Code != 1)
                            {
                                ...
                                UnitCard_Back_Office _UnitCard_Back_Office = Contexts2.UnitCard_Back_Office.FirstOrDefault(cat => cat.UnitCode == x.UnitCode && cat.IsDeleted == 0 && cat.Branch_Code == toBranchCode);
                            ...
                            UnitCard_Back_Office _cat = new UnitCard_Back_Office()
                            {
                            ...
                            Contexts2.UnitCard_Back_Office.Add(_cat);
                            Contexts2.SaveChanges();
        ...
        private void gvCategory_RowClick(object sender, RowClickEventArgs e)
        {
            try
            {
                var FocusRow = gvCategory.GetFocusedRow() as DataRep.UnitCard;

                labelControl1.Text = FocusRow.UnitName;
            }
            catch {
                labelControl1.Text = "";
            }
        ...
        private void simpleButton1_Click(object sender, EventArgs e)
        {
            ...
            if (String.IsNullOrWhiteSpace(labelControl1.Text)) {
                ...
                if (FocusRow != null)
             {
                ...
                _list_Unit_Back_Office = _list_Unit_Back_Office.Where(x => x.UnitCode != FocusRow.UnitCode).ToList();
                Int64 toBranchCode = Convert.ToInt64(slkToWarhouse.EditValue);
                var isFind = _server.UnitCard_Back_Office.Any(xx => xx.Branch_Code == toBranchCode && xx.IsDeleted == 0 && xx.UnitCode == FocusRow.UnitCode);



                var model = new UnitCard_Back_Office() { 
                
                Branch_Code = toBranchCode,
                UnitCode = FocusRow.UnitCode,
                UnitName = FocusRow.UnitName,
                IsDeleted = 0,
                ...
                _list_Unit_Back_Office.Add(model);

                gcToCategory.DataSource = _list_Unit_Back_Office.ToList();
       ...
       void fillToBranchGrid()
        {
                ...
                using (db_a8f74e_posEntities Contexts2 = new db_a8f74e_posEntities())
                {
                    _list_Unit_Back_Office = Contexts2.UnitCard_Back_Office.Where(x => x.IsDeleted == 0 && x.Branch_Code == toBranchCode).ToList();
                }
                  
                    gcToCategory.DataSource = _list_Unit_Back_Office;
                    gcToCategory.RefreshDataSource();
                }
                else
                {
                    labelControl1.Text = "";
                _list_Unit_Back_Office = new List<UnitCard_Back_Office>();
                    gcToCategory.DataSource = _list_Unit_Back_Office;
                    gcToCategory.RefreshDataSource();
                }
        ...
        private void slkToWarhouse_EditValueChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                gcToCategory.DataSource = _list_Unit_Back_Office.Where(x => x.IsDeleted == 0);
                gcToCategory.RefreshDataSource();
                fillToBranchGrid();
            }
            catch { 
        ...
        {
            var FocusRow = gvToCategory.GetFocusedRow() as UnitCard_Back_Office;
            ...
            if (FocusRow != null)
            {
                ...
                if (isFind)
                {
                    _list_Unit_Back_Office.FirstOrDefault(x => x.UnitCode == FocusRow.UnitCode).IsDeleted = 0;
                    _list_Unit_Back_Office.FirstOrDefault(x => x.UnitCode == FocusRow.UnitCode).IsWindow_Transfer = true;
                    _list_Unit_Back_Office.FirstOrDefault(x => x.UnitCode == FocusRow.UnitCode).Event_Code = 3;
                    gcToCategory.DataSource = _list_Unit_Back_Office;
                    gcToCategory.RefreshDataSource();
                }
                else
        ...
        private void toolStripMenuItem1_Click_1(object sender, EventArgs e)
        {
            ...
            if (FocusRow != null && FocusRow.IsWindow_Transfer)
            {
                ...
                {
                    _list_Unit_Back_Office.FirstOrDefault(x => x.UnitCode == FocusRow.UnitCode).IsDeleted = 0;
                    _list_Unit_Back_Office.FirstOrDefault(x => x.UnitCode == FocusRow.UnitCode).IsWindow_Transfer = false;
                    _list_Unit_Back_Office.FirstOrDefault(x => x.UnitCode == FocusRow.UnitCode).Event_Code = 0;
                    gcToCategory.DataSource = _list_Unit_Back_Office;
                    gcToCategory.RefreshDataSource();
                }
                else {
